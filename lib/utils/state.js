...
State.prototype = Object.assign(Object.create(Array.prototype), {
  ...
  resolveReference(reference) {
    ...
    const lastFullURISchema = reversed
    .find(schema => (
      uriKeys
      .map(url => schema[url])
      .find((url) => {
        lastFullURIReference = isFullUri(url) && url;
        return !!lastFullURIReference;
      })
    ));

    // collect all partial routes for it
    const lastFullURISchemaIndex = reversed.indexOf(lastFullURISchema);
    const partialReferences = reversed
      .slice(0, lastFullURISchemaIndex)
        .map((schema) => {
          let partialReference;
          uriKeys
            .map(url => schema[url])
            .find((url) => {
              partialReference = head(url) && url;
  ...
  ascend(reference) {
    ...
    // TODO make separate function ...resolved
    if (path && hasProperty(this.env.resolved, path)) {
      const { schema } = this.env.resolved[path];
      return schema;
    }
    ...
    if (path) {
      ...
      parentSchema = (
        ...
        this.env.resolved[resolvedId].schema
      ) ||
      this
        .slice()
        .reverse()
    ...
    while (
      parentSchema.$ref &&
      // avoid infinite loop
      cleanId(parentSchema.$ref) !== cleanId(reference) &&
