...
function template(state, options) {
  ...
  const error = typeof options.errorHandler === 'function' ?
    ...
    function defaultErrorHandler(errorType, errorValue) {
      const dataPath = this.data.toString()
        .replace(/decodeURIComponent\(([^)]+)\)/gi, '$1')
        .replace(/\[i([0-9]*)]/gi, '[$1]')
        .replace(/\['([^']+)']/gi, '.$1')
        .replace(/^data\.?(.*)/gi, '$1');
      const schemaPath = `${this.data.toString()}/${errorType}`
        .replace(/decodeURIComponent\(([^)]+)\)/gi, '$1')
        .replace(/\[i([0-9]*)]/gi, '/items')
        .replace(/\['([^']+)']/gi, '/properties/$1')
        .replace(/^data\/?(.*)/gi, '#$1');
