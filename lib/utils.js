...
function resolveUri(env, reference) {
    if(typeof reference !== 'string') {
        return reference;
    }
    ...
    // if(uri === '#') {
    //     currentSchema = states[states.indexes.length - 1];
    // } else {
    //     currentSchema = env[uri];
    // }
    // states.push(currentSchema);
    // states.indexes.push(states.length - 1);
