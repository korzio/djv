...
module.exports = function generate(
  env,
  schema,
  state = Object.assign([schema], { current: [0], context: [], entries: new Map() }, env.resolved)
) {
  const fn = factory();
  ...
  Object.assign(fn, {
    resolve(url) {
      ...
      if (typeof reference === 'object') {
        let generatedInnerFunction = state.entries.get(reference);
        if (!generatedInnerFunction) {
          generatedInnerFunction = generate(env, reference, state).toFunction(true);
          state.entries.set(reference, generatedInnerFunction);
        }

        reference = state.context.push(generatedInnerFunction);
        if (changedState) {
